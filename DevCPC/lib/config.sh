...
load_config() {
    if [[ ! -f "devcpc.conf" ]]; then
        error "No se encontró devcpc.conf"
        error "¿Estás en un proyecto DevCPC?"
        error "Usa 'devcpc new <nombre>' para crear uno nuevo"
        exit 1
    fi
    ...
    if [[ -z "$PROJECT_NAME" ]]; then
        error "PROJECT_NAME no está definido en devcpc.conf"
        exit 1
    fi
